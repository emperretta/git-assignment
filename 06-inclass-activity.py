...
if run:
    ...
    comparison_df['pct_change'] = comparison_df['Close'].pct_change()
    ...
    with tab4:
        st.subheader(f"{ticker} vs {comparison_ticker} Comparison")
        ...
        # Create chart
        figure = px.line(combined_df, y=[ticker, comparison_ticker])
        figure.update_layout(title=f"{ticker} vs {comparison_ticker} Performance (Base 100)")
        st.plotly_chart(figure, use_container_width=True)
